...
// Pattern from strip-turbowarp.cjs
const regex1 = /<script>\s*\(function \(\) \{\s*var\s+[a-zA-Z0-9]+\s*=\s*'[^']*',\s*[a-zA-Z0-9]+\s*=\s*[0-9]+\s*-\s*[0-9]+;[\s\S]*?\}\)\(\)\s*<\/script>/;

console.log("Regex 1 Match:", regex1.test(content));

// Try a broader regex
const regex3 = /<script>\s*\(function\s*\(\)\s*\{[\s\S]*?function\s+xsh\s*\([\s\S]*?\}\)\(\)\s*<\/script>/;
console.log("Regex 3 Match:", regex3.test(content));
