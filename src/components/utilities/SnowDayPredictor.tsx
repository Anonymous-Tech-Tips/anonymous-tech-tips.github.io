...
interface ForecastData {
    forecasts: DistrictForecasts['forecasts'];   // LCPS fallback (backward compat)
    districts: Record<string, DistrictForecasts>;
    last_updated: string;
}
...
export const SnowDayPredictor = () => {
    ...
    // Hardcoded fallback — used if the fetch fails before first GitHub Actions run
    const FALLBACK_FORECAST: ForecastData = {
        forecasts: [
            { date: "2026-03-03", label: "Tuesday, Mar 03", primary_prediction: "Delay", probabilities: { open: 29.8, delay: 56.9, early_release: 4.6, closure: 8.7 } },
            { date: "2026-03-04", label: "Wednesday, Mar 04", primary_prediction: "Open", probabilities: { open: 68.5, delay: 18.0, early_release: 5.5, closure: 8.0 } },
            { date: "2026-03-05", label: "Thursday, Mar 05", primary_prediction: "Open", probabilities: { open: 80.0, delay: 10.0, early_release: 3.0, closure: 7.0 } }
        ],
        districts: {
            LCPS: {
                forecasts: [
            ...
            FCPS: {
                forecasts: [
                    { date: "2026-03-03", label: "Tuesday, Mar 03", primary_prediction: "Open", probabilities: { open: 65.0, delay: 22.0, early_release: 5.0, closure: 8.0 } },
                    { date: "2026-03-04", label: "Wednesday, Mar 04", primary_prediction: "Open", probabilities: { open: 72.0, delay: 16.0, early_release: 5.0, closure: 7.0 } },
                    { date: "2026-03-05", label: "Thursday, Mar 05", primary_prediction: "Open", probabilities: { open: 80.0, delay: 10.0, early_release: 4.0, closure: 6.0 } }
                ]
            },
            PWCS: {
                forecasts: [
                    { date: "2026-03-03", label: "Tuesday, Mar 03", primary_prediction: "Open", probabilities: { open: 70.0, delay: 18.0, early_release: 5.0, closure: 7.0 } },
                    { date: "2026-03-04", label: "Wednesday, Mar 04", primary_prediction: "Open", probabilities: { open: 75.0, delay: 14.0, early_release: 4.0, closure: 7.0 } },
                    { date: "2026-03-05", label: "Thursday, Mar 05", primary_prediction: "Open", probabilities: { open: 82.0, delay: 9.0, early_release: 3.0, closure: 6.0 } }
                ]
            }
        },
        last_updated: "2026-03-03 08:00:00"
    };
    ...
    // Read the correct district's forecasts from the per-district structure,
    // falling back to the top-level `forecasts` array (LCPS) for backward compat
    const districtForecasts =
        data?.districts?.[selectedDistrict.id]?.forecasts ?? data?.forecasts ?? [];
    const forecast = districtForecasts[dayIndex];
            ...
            <div className="relative z-10 p-6 md:p-10 min-h-[600px] flex flex-col">
                ...
                <AnimatePresence mode="wait">
                    {loading ? (
                        ...
                        <motion.div
                            key={`result-${selectedDistrict.id}-${dayIndex}`}
